...
- **ปุ่ม Save และ Popup ยืนยัน**:
  ...
  - **Popup เตือนเลขที่เอกสารข้าม (Save Confirmation Prompt)**:
    - ข้อความ: `เลขที่เอกสารข้ามเลขที่ ... ต้องการบันทึกหรือไม่ ?`
    - Class: `#32770`, Title: `คำเตือน`
    - ปุ่ม: `[ Yes ]` (CtrlId=6 / `IDYES`) และ `[ No ]` (CtrlId=7 / `IDNO`)
    - ระบบจะตรวจจับข้อความ `ข้ามเลขที่` / `ต้องการบันทึกหรือไม่` อัตโนมัติ และเลือกกดปุ่ม **Yes** (ทั้งผ่าน `BM_CLICK`, คลิกพิกัดปุ่ม, และส่งคีย์ `Y` + `Enter`) พร้อมรอจน Dialog ปิด และรองรับ Popup สำเร็จที่ตามมา (เช่น "บันทึกเรียบร้อย") อัตโนมัติ
- **Popup เตือนเลขที่ใบส่งของเป็นค่าซ้ำ (Duplicate Delivery Order Handling)**:
    - ข้อความ: `เลขที่ใบส่งของ เป็นค่าซ้ำ !!`
    - Class: `#32770`, Title: `คำเตือน`
    - ปุ่ม: `[ OK ]` (CtrlId=1 / `IDOK`)
    - พฤติกรรม Prosoft: เมื่อกด `TAB` ออกจากช่องเลขที่ใบส่งของ PowerBuilder จะเช็คกับฐานข้อมูลทันที หากซ้ำจะเปิด Dialog เตือนและเมื่อกด `OK` จะโฟกัสและไฮไลต์ข้อความเดิมในช่อง
    - การจัดการอัตโนมัติ: ระบบจะตรวจจับ Popup ข้อความ `เลขที่ใบส่งของ เป็นค่าซ้ำ !!`, กดปุ่ม **OK** เพื่อปิด Popup แล้ววนลูปเติม suffix `/1`, `/2`, `/3` ... ไปเรื่อยๆ จนกว่าจะไม่ขึ้นค่าซ้ำและบันทึกข้อมูลเข้าเอกสาร
- **Popup เตือนตัดเงินมัดจำ (Deposit Deduction Prompt)**:
    - ข้อความ: `ท่านมีจำนวนเงินตัดมัดจำต้องการใช้หรือไม่`
    - Class: `#32770`, Title: `คำเตือน`
    - ปุ่ม: `[ Yes ]` (CtrlId=6 / `IDYES`) และ `[ No ]` (CtrlId=7 / `IDNO`)
    - การจัดการอัตโนมัติ: ระบบตรวจจับข้อความ `ตัดมัดจำ` / `เงินตัดมัดจำ` และเลือกกดปุ่ม **No** อัตโนมัติ เพื่อไม่นำเงินมัดจำมาตัดในเอกสารนี้ และรอจนกว่ากระบวนการบันทึกจะเสร็จสมบูรณ์
